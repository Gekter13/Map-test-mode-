...
var firstTimeFunctionMarkers = false;
var firstTimeFunctionGrups = false;
		...
		if (HTTPstatus === 200) {
            ...
            for (var i=0; i<cli.length; i++){
                idUsers[i]={
                    "id": cli[i].ID,
                    "Title": cli[i].Title,
                    "client": JSON.stringify({"Client": cli[i].ID}),
                }
                c = JSON.stringify({
                "Client": cli[i].ID
            }); 
                
            var userId = String(cli[i].ID); 
            //alert(c)
                }
                globalMap();
               }
...
/*function getCarsUsers(clientIdText) {
    dc.post('TruckingSystem/GetGroups', clientIdText, function(HTTPstatus, response, passedOptions) {
        if (HTTPstatus === 200) {
            ...
            dc.post('TruckingSystem/GetUnits', clientIdText, function(HTTPstatus, response, passedOptions) {
        if (HTTPstatus === 200) {
...
function globalMap() {
    ...
    (function() {
        ...
        tableUser.className = 'tabUser';
        var q = "<table class='table table-striped' id='info1-table'>\
				    <tr>\
                        <th>Пользователи</th>\
					<tr>";
        for (var t=0; t<idUsers.length; t++){
            q = q + '<tr>\
                    <td><div class="dropdownUser "id=' + "S" + idUsers[t].id + ' ><button class="dropUsers" value=' + idUsers[t].id + '>' + idUsers[t].Title + '</button><div id=' + idUsers[t].id + " " + idUsers[t].Title + ' class="UserDiv"></div></div></td>\
            ';
        }
        q = q + '</table>';
        tableUser.innerHTML = q;
        stuffsBlock.appendChild(tableUser);
        
        
        /* for(var r = 0; r < idUsers.length; r++) {
        
            var clientIdText =  idUsers[r].client;
       
        
            //getCarsUsers(clientIdText);
     ...
     ';
        ...
        //tableUser.appendChild(tableStuffs);
        var divTableGrup = document.getElementById(String(idUsers[r].id));
        divTableGrup.innerHTML = f;
        ...
        stuffsBlock.querySelectorAll(".dropUsers").forEach(el => {
            el.addEventListener('click', function(e) {
                if (!e.target.matches('.idUsers')) {
                    let itemq = idUsers.find(n => {
                        return n.id == e.target.value;
                    });
                    
                    
            if($('#'+itemq.id).children('#info-table').length === 0){
            //alert('1 вызов');
            
            var cliId = JSON.stringify({
                "Client": itemq.id
            });
                   
            dc.post('TruckingSystem/GetGroups', cliId, function(HTTPstatus, response, passedOptions) {
        if (HTTPstatus === 200) {
            var obj = JSON.parse(response).result[0];
            for (var i = 0; i < obj.length; i++) {
                idParents[i] = {
            ...
            dc.post('TruckingSystem/GetUnits', cliId, function(HTTPstatus, response, passedOptions) {
        if (HTTPstatus === 200) {
            obj = JSON.parse(response).result[0];
            for (var i = 0; i < obj.length; i++) {
                TrackerIDs[i] = obj[i].ID;
         ...
         //class='table table-striped'   
            ...
            for (var i = 0; i < stuffs.length; i++) {
                if (stuffs[i].parentsId === 0) {
                    ...
                    <td><input type="checkbox" id = ' + 'after' + stuffs[i].id + ' class="checkboxClass" name="stuffs" value=' + stuffs[i].id + '></td>\
                ';
                    const divNoPar = document.getElementById(String(itemq.id) + 'noGrup');
        ...
        for (var i = 0; i < idParents.length; i++) {
            if (idParents[i].PpId != 0) {
                f = f + '<tr>\
                    <td><div class="dropdownPar "id=' + "S" + idParents[i].PpId + ' ><button id=' + "butUnder" + idParents[i].id + ' class="dropParents" value=' + idParents[i].id + '>' + idParents[i].Title + '</button><div id=' + idParents[i].id + " " + idParents[i].Title + ' class="parentDiv"></div></div></td>\
     ';
            } else {
                f = f + '<tr>\
                    <td><div class="dropdownPar" ><button id=' + "but" + idParents[i].id + ' class="dropParent" value=' + idParents[i].id + '><img src="images/plus-square-regular.svg" alt="" class="classImageCar">' + idParents[i].Title + '</button><div id=' + idParents[i].id + " " + idParents[i].Title + ' class="parentDiv"></div></div></td>\
     ';
            }
        }
        f = f + '</table>';
        //tableStuffs.innerHTML = f;
        //tableUser.appendChild(tableStuffs);
        var divTableGrup = document.getElementById(itemq.id);
        divTableGrup.innerHTML = f;
        ...
        fnoGr = fnoGr + '<tr>\
                    <td><div class="dropdownNoPar"><button class="dropNoParent" value=' + itemq.id+ '><img src="images/plus-square-regular.svg" alt="" class="classImageCar">' + itemq.Title + '(без груп)'+'</button><div id=' + itemq.id + 'noGrup' + ' class="parentNoDiv"></div></div></td>\
     ';
                

        fnoGr = fnoGr + '</table>';
             parentNoGrup.innerHTML = fnoGr;
             divTableGrup.appendChild(parentNoGrup);
        //divTableGrup.innerHTML = fnoGr;
       ...
       /* for (var j = 0; j < idParents.length; j++) {
            ...
            for(var i=0; i<idParents.length; i++){
               $('#but'+idParents[i].id).addClass("myClass");  
             }      
        ...
        startFintNoGrups(hId);
            ...
            $('#'+idUsers[i].id).css( "display", "none" );
            }    
            
            $('#'+itemq.id).css( "display", "block" );
            ...
            //alert('2 вызов');
            for(var i=0; i<idParents.length; i++){
               $('#but'+idParents[i].id).removeClass("myClass"); 
                document.getElementById(idParents[i].id).classList.remove('showPar');
                    
            }   
            document.getElementById(String(itemq.id)+"noGrup").classList.remove('showParNo');
            idParents.length = 0;
        ...
        if (HTTPstatus === 200) {
            ...
            for (var i = 0; i < obj.length; i++) {
                ...
                if($('#'+itemq.id).css('display') == 'block'){
                   $('#'+itemq.id).css( "display", "none" );
                   //for(var y=0; y<idParents.length; y++){} 
                }
                else {
                   for(var i=0; i<idUsers.length; i++){
                    $('#'+idUsers[i].id).css( "display", "none" );}   
        ...
        const stuffsBlockUser = document.querySelector('body').querySelector('#sidebar').querySelector('.sidebar-content').querySelector('#home').querySelector('#optionId').querySelector('.tabUser');
...
`;
       ...
       /* const tableUser = document.createElement('div');
        ...
        /* 
        Оборачиваем создание tableStuffs в функцию и запускаем ее в цикле getListName для каждого клиениа что есть
        */
        

       /* function createGrupsInUserGrup(userId) {
     ...
     ';
        ...
        //tableStuffs.innerHTML = f;
        //tableUser.appendChild(tableStuffs);
        var divTableGrup = document.getElementById(userId);
        divTableGrup.innerHTML = f;
     ...
     ';
        ...
        */
        ...
        if(idUsers.length <= 1 ){
            ...
            $('.dropUsers').trigger('click');
            }, 1000);
            //document.getElementById(nameUser).classList.toggle("showParNo");
           }
        
        
        //группы машин без групп
        function startFintNoGrups(hId) {
        $('.dropNoParent').click(function(e) {
            //alert('popal');
            e.stopPropagation();
            arrayOfIds.length = 0;
            var selectedItem = document.getElementById(String(hId)+"noGrup");
            if (selectedItem.classList.contains('showPar')) {
                document.getElementById(String(hId)+"noGrup").classList.remove('showPar');
                $('.underMenu').remove();
            } else {
                ...
                document.getElementById(String(hId)+"noGrup").classList.toggle("showParNo");

                arrayOfIds = $.map($('.checkboxClass', '#' + String(hId)+"noGrup"), function(n, i) {
        ...
        //группы машин
        function startFintGrups(h) {
        stuffsBlockUser.querySelectorAll('.myClass').forEach(el => {
            el.addEventListener('click', function(e) {
                if (!e.target.matches('.idParents')) {
                    ...
                    if(firstTimeFunctionMarkers===false) {
                    firstTimeFunctionMarkers = true;
                    //startFindMarkers();
                    } else {}
                    ...
                    if (selectedItem.classList.contains('showPar')) {
                        ...
                        for (var k = 0; k < arrayOfIds.length; k++) {
                            if (sessionStorage.getItem('afterCar' + arrayOfIds[k].id) === "true") {
                            $('#afterCar' + arrayOfIds[k].id).prop('checked', true);
                            }
                            sessionStorage.removeItem(arrayOfIds[k]);
                        }
                        ...
                        if ($(".parentNoDiv").length > 0 ) {
                        //document.getElementsByClassName("parentNoDiv").classList.remove("showParNo");
                        $('.parentNoDiv').removeClass('showParNo')
                        }
                        else {}
        ...
        stuffsBlockUser.querySelectorAll(".dropParents").forEach(el => {
            el.addEventListener('click', function(e) {
                if (!e.target.matches('.idParents')) {
                    let itemq = idParents.find(n => {
                        return n.id == e.target.value;
        ...
        //маркеры
        function startFindMarkers(){
            
        stuffsBlockUser.querySelectorAll(".checkboxClass").forEach(el => {
        ...
        stuffsBlockUser.querySelectorAll(".checkboxnewrem").forEach(el => {
